...
namespace Citicon.Inventory.DataManager
{
    public sealed class TransactionManager : DataManager<Transaction>, IDataManager<Transaction>
    {
        ...
        {
            if (NewItemTransactionGenerated != null) NewItemTransactionGenerated(transaction);
        }
        ...
        public void ExportIssuanceSlip(Transaction transaction)
        {
            if (File.Exists(IssuanceSlipTemplate))
            {
                ...
                try
                {
                    sheet.Cells[1, 9] = transaction.Id.ToString("0000000");
                    ...
                    sheet.Cells[14, 4] = transaction.Item?.Classification?.Description;
                    sheet.Cells[15, 4] = transaction.Item?.SubClassification?.Description;
                    sheet.Cells[16, 4] = transaction.Company?.Description;
                    sheet.Cells[17, 4] = transaction.Branch?.Description;
                    sheet.Cells[18, 4] = transaction.RemovedStockValue;
                    sheet.Cells[20, 4] = transaction.Purpose;
                    sheet.Cells[24, 1] = transaction.RequestedBy;
                    sheet.Cells[24, 6] = transaction.ReleasedBy;
        ...
        public Transaction ExtractFromDictionary(Dictionary<string, object> dictionary)
        {
            if (dictionary != null)
            {
                return new Transaction
                {
                    ...
                    RemovedStockValue = dictionary.GetUInt32("RemovedStockValue"),
