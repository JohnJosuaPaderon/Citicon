...
{
    public static class SpecialHolidayWorkingHoursCounter
    {
        public static async Task<decimal> CountAsync(Employee employee, DateTimeRange cutOff)
        {
            ...
            var timeLogs = await PayrollManager.GetTimeLogListAsync(employee, TimeLogType.SpecialHolidayWorkingHours, cutOff);
            ...
            if (timeLogs != null && timeLogs.Any())
            {
                ...
                else
                {
                    ...
                    foreach (var timeLog in cleansed)
                    {
                        ...
                        result += Convert.ToDecimal(totalHours <= 0 ? 0 : Math.Round(totalHours, 2) - 1);
                    }
        ...
        {
            return loginDate?.Date.AddHours(8) ?? default(DateTime);
        }
