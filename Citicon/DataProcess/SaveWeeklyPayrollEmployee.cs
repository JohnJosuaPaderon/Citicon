...
{
    internal sealed class SaveWeeklyPayrollEmployee : DataProcessBase
    {
        public SaveWeeklyPayrollEmployee(WeeklyPayrollEmployee payrollEmployee)
        {
            _PayrollEmployee = payrollEmployee ?? throw new ArgumentNullException(nameof(payrollEmployee));
        }

        private readonly WeeklyPayrollEmployee _PayrollEmployee;

        private MySqlCommand CreateCommand(MySqlConnection connection, MySqlTransaction transaction)
        {
            return Utility.CreateProcedureCommand(nameof(SaveWeeklyPayrollEmployee), connection, transaction)
                .AddOutParameter("@_Id")
        ...
        private WeeklyPayrollEmployee Callback(int affectedRows, MySqlCommand command)
        {
            if (affectedRows > 0)
            {
        ...
        public Task<WeeklyPayrollEmployee> ExecuteAsync(MySqlConnection connection, MySqlTransaction transaction)
        {
